...
use Logliy\Symfony\Polyfill\Php84 as p;
...
if (\extension_loaded('curl')) {
    ...
    if (defined('CURL_VERSION_HTTP3') || \PHP_VERSION_ID < 80200 && curl_version()['version_number'] >= 0x074200) {
        if (!defined('LOGLIY_CURL_HTTP_VERSION_3')) {
            define('LOGLIY_CURL_HTTP_VERSION_3', 30);
        }

        // CURL_HTTP_VERSION_3ONLY requires libcurl >= 7.88.0 and is not gated by any PHP-defined constant before 8.4
        if (!defined('LOGLIY_CURL_HTTP_VERSION_3ONLY') && curl_version()['version_number'] >= 0x075800) {
            define('LOGLIY_CURL_HTTP_VERSION_3ONLY', 31);
        }
    }
}

if (!function_exists('logliy_array_find')) {
    function logliy_array_find(array $array, callable $callback) { return p\Php84::array_find($array, $callback); }
}

if (!function_exists('logliy_array_find_key')) {
    function logliy_array_find_key(array $array, callable $callback) { return p\Php84::array_find_key($array, $callback); }
}

if (!function_exists('logliy_array_any')) {
    function logliy_array_any(array $array, callable $callback): bool { return p\Php84::array_any($array, $callback); }
}

if (!function_exists('logliy_array_all')) {
    function logliy_array_all(array $array, callable $callback): bool { return p\Php84::array_all($array, $callback); }
}

if (!function_exists('logliy_fpow')) {
    function logliy_fpow(float $num, float $exponent): float { return p\Php84::fpow($num, $exponent); }
}
...
if (extension_loaded('bcmath')) {
    ...
    if (!function_exists('logliy_bcdivmod')) {
        function logliy_bcdivmod(string $num1, string $num2, ?int $scale = null): ?array { return p\Php84::bcdivmod($num1, $num2, $scale); }
    }

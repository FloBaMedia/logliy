...
namespace Logliy\Symfony\Component\Serializer\Encoder;

use Logliy\Symfony\Component\Serializer\Exception\BadMethodCallException;
use Logliy\Symfony\Component\Serializer\Exception\NotEncodableValueException;
        ...
        if (\is_array($data) || ($data instanceof \Traversable && (null === $this->serializer || !$this->serializer->supportsNormalization($data, $format)))) {
            foreach ($data as $key => $data) {
                ...
                if (str_starts_with($key, '@') && $this->isElementNameValid($attributeName = substr($key, 1))) {
                    ...
                    if (!$preserveNumericKeys && $data && null === logliy_array_find_key($data, static fn ($v, $k) => \is_string($k))) {
                        /*
